"""
COMPLETE AI WASTE MANAGEMENT SYSTEM
Merged: Advanced Features (Working Predictions) + Login/Signup + Multi-Object Detection
"""
...
def init_database():
    ...
    if not c.fetchone():
        c.execute("INSERT INTO users (username, email, password, role, department, created_at) VALUES (?,?,?,?,?,?)",
                  ('admin', 'admin@system.com', hashlib.sha256('admin123'.encode()).hexdigest(), 'admin', 'IT', datetime.now().isoformat()))
        c.execute("INSERT INTO users (username, email, password, role, department, created_at) VALUES (?,?,?,?,?,?)",
                  ('worker1', 'worker1@system.com', hashlib.sha256('worker123'.encode()).hexdigest(), 'worker', 'Operations', datetime.now().isoformat()))
        c.execute("INSERT INTO users (username, email, password, role, department, created_at) VALUES (?,?,?,?,?,?)",
                  ('manager1', 'manager1@system.com', hashlib.sha256('manager123'.encode()).hexdigest(), 'manager', 'Facilities', datetime.now().isoformat()))
...
def log_detection(user_id, username, waste_type, confidence, contaminated, decision):
    conn = sqlite3.connect("waste_system.db")
    c = conn.cursor()
    c.execute("INSERT INTO detections (user_id, username, waste_type, confidence, contaminated, decision, timestamp) VALUES (?,?,?,?,?,?,?)",
...
# ============================================
# MULTI-OBJECT DETECTOR (Using YOLO if available)
# ============================================
...
if not st.session_state.logged_in:
    ...
    with col2:
        ...
        with tab2:
            ...
            signup_department = st.selectbox("Department", ["General", "Facilities", "Operations", "IT"])
            ...
            if st.button("Sign Up", type="primary", use_container_width=True):
                if signup_password != signup_confirm:
                    ...
                    success, msg = create_user(signup_username, signup_email, signup_password, signup_role, signup_department)
                    if success:
...
<div class="main-header">
    ...
    <p>👤 {st.session_state.username} ({st.session_state.role}) | Department: {st.session_state.user['department']}</p>
...
# ============================================
# TAB 5: BATCH PROCESSING (if available)
# ============================================
